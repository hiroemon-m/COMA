...
plt.title("Attribute NLL (DBLP)", fontsize=30)

# 横軸を定義
left = np.array([1, 2, 3, 4, 5])


proposed_attr_data = np.load("proposed_attr_dblp_nll.npy")
m1_attr = proposed_attr_data.mean(axis=0)
std1_attr = proposed_attr_data.std(axis=0)


proposed_attr_only_data = np.load("attr_proposed_dblp_only_nll.npy")
