...
plt.title("Edge NLL (NIPS)", fontsize=18)

# 横軸を定義
left = np.array([1, 2, 3, 4, 5])

proposed_data = np.load("proposed_edge_nips_nll.npy")
m1 = proposed_data.mean(axis=0)
std1 = proposed_data.std(axis=0)

proposed_attr_data = np.load("proposed_attr_nips.npy")
...
gcn_data = np.load("gcn_nips.npy")
gcn_m1 = gcn_data.mean(axis=0)
gcn_std1 = gcn_data.std(axis=0)


vgrnn_data = np.load("vgrnn_nips.npy")
