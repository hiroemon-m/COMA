...
class Actor(nn.Module):
    ...
    def predict(self,attributes, edges,time):
        ...
        for i in range(len(self.persona[0][0])):
            ...
            tmp_tensor = self.W[i] * torch.matmul(edges, attributes) #+ trend
            feat_action = self.r[i] * attributes + tmp_tensor * (1-self.r[i]) 
            feat_action = torch.clamp(feat_action,max=5)       
            feat_tanh = torch.tanh(feat_action)
        ...
        return edges_prob,edge_action, feat_action
