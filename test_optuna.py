...
class COMA:
    def __init__(self, obs,agent_num,input_size, action_dim, lr_c, lr_a, gamma, target_update_steps,T,e,r,w,rik,story_count,lr_t,lr_e,lr_r,lr_w):
        ...
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=lr_c, weight_decay=5e-4)
        self.critic_before_optimizer = torch.optim.Adam(self.critic_before.parameters(), lr=lr_c, weight_decay=5e-4)
        self.ln = 0
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            self.actor.T -= self.lr_t* self.actor.T.grad
            self.actor.e -= self.lr_e * self.actor.e.grad
            self.actor.r -= self.lr_r * self.actor.r.grad
            self.actor.W -= self.lr_w * self.actor.W.grad
...
def execute_data(trial):
    ...
    alpha = means[:,0]
    beta = means[:,1]
    gamma =  trial.suggest_float("p_gamma", 0.1, 0.90, log=True)
    lr_a =  trial.suggest_float("lr", 1e-6, 1e-1, log=True)
    lr_c =  trial.suggest_float("lr", 1e-6, 1e-1, log=True)
    lr_t = trial.suggest_float("lr", 1e-6, 1e-1, log=True)
    lr_e = trial.suggest_float("lr", 1000, 10000, log=True)
    lr_r = trial.suggest_float("lr", 1e-6, 1e-1, log=True)
    lr_w = trial.suggest_float("lr", 1e-6, 1e-1, log=True)
    ...
    while flag or ln_sub <= 1:
        ...
        episode_reward = 0
        episodes_reward = []
        agents = COMA(obs,agent_num, input_size, action_dim, lr_c, lr_a, gamma, target_update_steps,T,e,r,w,mixture_ratio,story_count,lr_t,lr_e,lr_r,lr_w)
...
if __name__ == "__main__":
    ...
    # print("Best Average AUC:", study.best_value)
