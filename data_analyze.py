...
for time in range(TOTAL_TIME):
    ...
    print("-----------------{}----------------".format(time))
    print("属性値の総数")
    print(torch.sum(attr>0))
    print(torch.sum(attr>0,dim=1))
    ...
    if time >= 5:
        ...
        #print(ta)
        ta = torch.sum(attr_before,dim=0)
        trend = (ta).repeat(32,1)

        trend = torch.where(trend>0,1,0)
        ta = (attr>0)&(trend>0)

        nta = torch.sum(torch.where((torch.where(trend>0,1,0)) - (torch.where(attr<0,1,0))>0,1,0))
        
        print("{}流行の属性値".format(time),torch.sum(ta))
        print("not{}流行の属性値".format(time),torch.sum(nta))
        trend = torch.sum(attr_before,dim=0).repeat(32,1)
        #print("ysoku",torch.sum(torch.where((attr_before+adj+trend)>0.3,1,0)))
