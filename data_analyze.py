...
for time in range(TOTAL_TIME):
    ...
    if time >= 1:
        ...
        print("一つ前の時刻と同じ属性値")
        print(torch.sum((attr>0.0)&(attr_before>0.0),dim=1))
        print("一つ前の時刻の隣接ノード同じ属性値")
        adj = torch.matmul(edge_before,attr_before)
        print(torch.sum((adj>0.0)&(attr>0.0),dim=1))
       ...
       # print("一つ前の時刻で類似しているノードと同じ属性値")
        ...
        trend = (torch.sum(attr_before,dim=0)>0).repeat(32,1)
        trend = torch.where(trend>0,1,0)
        print("{}流行の属性値".format(time),torch.sum((attr>0)&(trend),dim=1))
