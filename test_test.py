...
class PPO:
    def __init__(self, obs,agent_num,input_size, action_dim, gamma,T,e,r,w,rik,temperature,story_count,data_set):
...
def execute_data(persona_num,data_name,data_type):
    ...
    print("per",persona_ration.get_device())
    print("mean",means.get_device())
    ...
    #パラメータ
    mu = 0.32
    lr =1.4731827325595173e-05
    temperature = 0.01


    
    T = torch.tensor([1.0253 for _ in range(persona_num)], dtype=torch.float32).to(device)
    e = torch.tensor([1.108 for _ in range(persona_num)], dtype=torch.float32).to(device)
    r = torch.tensor([0.788 for _ in range(persona_num)], dtype=torch.float32).to(device)
    w = torch.tensor([0.928 for _ in range(persona_num)], dtype=torch.float32).to(device)
    ...
    for count in range(10):
        ...
        for test_time in range(TOTAL_TIME - GENERATE_TIME):
            ...
            plt.ylabel('TPR: True positive rate')
            plt.grid()
            plt.savefig('edge_sklearn_roc_curve.png'.format(data_name,data_type,persona_num))
            #print("-------")
