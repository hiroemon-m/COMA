    ...
    def train(self,gamma):
        ...
        for i in range(2):
            ...
            for i in range(storycount):
                ...
                loss = loss - torch.sum(torch.log(new_policy[i]+1e-5)*reward)
...
def execute_data(traial):
    ...
    path_n = "optimize/{}/{}/persona={}/".format(data_type,data_name,persona_num)
    path = path_n+"gamma.npy"
    ...
    mu = traial.suggest_float("mu",0.01,0.9,log=True)
    lr =  traial.suggest_float("lr",1e-6,1e-1,log=True)
    def_t = traial.suggest_float("T",0.5,1.5,log=True)
    def_e = traial.suggest_float("e",0.5,1.5,log=True)
    def_r = traial.suggest_float("r",1e-3,1.0,log=True)
    def_w = traial.suggest_float("w",1e-3,1.0,log=True)
    temperature = 0.01

    
    
    T = torch.tensor([def_t for _ in range(persona_num)], dtype=torch.float32)
    e = torch.tensor([def_e for _ in range(persona_num)], dtype=torch.float32)
    r = torch.tensor([def_r for _ in range(persona_num)], dtype=torch.float32)
    w = torch.tensor([def_w for _ in range(persona_num)], dtype=torch.float32)
    ...
    for count in range(10):
        ...
        for test_time in range(TOTAL_TIME - GENERATE_TIME):
            ...
            agents.memory.clear()
    return  np.mean((attr_calc_log+calc_log)/2)

    

    np.save("proposed_edge_auc".format(data_name,data_type,persona_num), calc_log)
    np.save("proposed_edge_nll".format(data_name,data_type,persona_num), calc_nll_log)
    np.save("proposed_attr_auc".format(data_name,data_type,persona_num), attr_calc_log)
    np.save("proposed_attr_nll".format(data_name,data_type,persona_num), attr_calc_nll_log)
    print("t",T,"e",e,"r",r,"w",w)
    print(mixture_ratio)
    np.save("persona_ration".format(data_name,persona_num),np.concatenate([mixture_ratio.detach().numpy()],axis=0))
    np.save("paramerter".format(data_name,persona_num),np.concatenate([T.detach().numpy(),e.detach().numpy(),r.detach().numpy(),w.detach().numpy()],axis=0))
...
if __name__ == "__main__":
    ...
    for i in [5]:
        ...
        study.optimize(execute_data,n_trials=30)
        #execute_data(i,"NIPS","complete")
        print("best_param",study.best_trial.params)